...
public abstract class BaseTrade implements Runnable {
    ...
    private IOrderSellTradeService scoreOrderSellTradeService;
     ...
     * @return
     */
    public boolean loadAllOrdersFromDB(int plateType) {
        try {
            // 加载积分订单
            List<OrderScoreInfo> scoreOrders = orderBusinessService.queryAllPositionScoreOrders(plateType);
            if (scoreOrders != null) {
                for (OrderScoreInfo scoreOrder : scoreOrders) {
                    TradeDirectVo tradeDirect = new TradeDirectVo(scoreOrder.getTradeDirection(), TradeTypeEnum.BUY.getValue());
                    RiskControlBean riskBean = new RiskControlBean();
                    riskBean.setProductName(scoreOrder.getProductCode());
                    riskBean.setUniqueOrderId(scoreOrder.getOrderId());
                    riskBean.setDirect(tradeDirect.getTradeDirect());
                    riskBean.setMatchPrice(scoreOrder.getBuyAvgPrice() == null ? 0.0 : scoreOrder.getBuyAvgPrice());
                    riskBean.setStopGainPrice(scoreOrder.getStopProfitPrice());
                    riskBean.setStopLossPrice(scoreOrder.getStopLossPrice());
                    riskBean.setMutableStopLoss(scoreOrder.getTrailStopLoss() == LTConstants.MUTABLE_STOPLOSS_OPEN);
                    riskBean.setDeferredOrderTimeStamp(scoreOrder.getSysSetSellDate().getTime());
                    FutureOrderBean orderBean = new FutureOrderBean(scoreOrder.getUserId(), "9999", scoreOrder.getExchangeCode(), scoreOrder.getProductCode(),
                            scoreOrder.getOrderId(), 888888, scoreOrder.getBuyEntrustCount(), scoreOrder.getBuyAvgPrice() == null ? 0.0 : scoreOrder.getBuyAvgPrice(),
                            ...
                            scoreOrder.getCreateDate().getTime());
    ...
    public void setScoreOrderSellTradeService(IOrderSellTradeService orderSellTradeService) {
        this.scoreOrderSellTradeService = orderSellTradeService;
    }

    public IOrderSellTradeService getScoreOrderSellTradeService() {
        return scoreOrderSellTradeService;
    }
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                for (FutureOrderBean futureOrder : orders) {
                    ...
                    if (futureOrder.getFundType() == LTConstants.FUND_TYPE_SCORE){
                        try {
                            orderBean.setFundType(LTConstants.FUND_TYPE_SCORE);
                            scoreOrderSellTradeService.doSell(orderBean);
                            LOGGER.info("风控平仓>> 账号: {}, 积分订单Id: {}, 商品: {}, 方向: {}, 数量: {},平仓类型：{}",
                            ...
                            LOGGER.error("---------LTException:{},futureScoreOrder:{}--------", e, JSONObject.toJSONString(futureOrder));
                            continue;
                        } catch (Exception e) {
                            e.printStackTrace();
                            LOGGER.error("---------error:{},futureScoreOrder:{}--------", e, JSONObject.toJSONString(futureOrder));
                            OrderScoreInfo scoreOrder = orderBusinessService.findOrderScoreInfoByOrderId(futureOrder.getUniqueOrderId());
                            if (StringTools.isNotEmpty(scoreOrder)) {
                                //风控触发失败，删除卖出标志
                                TradeUtils.delKeyLockSell(redisTemplate, scoreOrder.getOrderId());
                                RiskControlBean riskBean = new RiskControlBean();
                                riskBean.setProductName(scoreOrder.getProductCode());
                                riskBean.setUniqueOrderId(scoreOrder.getOrderId());
                                ...
                                riskBean.setMutableStopLoss(scoreOrder.getTrailStopLoss() == LTConstants.MUTABLE_STOPLOSS_OPEN);
                                riskBean.setDeferredOrderTimeStamp(scoreOrder.getSysSetSellDate().getTime());
                                FutureOrderBean oBean = new FutureOrderBean(scoreOrder.getUserId(), scoreOrder.getSecurityCode(), scoreOrder.getExchangeCode(), scoreOrder.getProductCode(),
                                        scoreOrder.getOrderId(), 888888, scoreOrder.getBuyEntrustCount(), scoreOrder.getBuyAvgPrice() == null ? 0.0 : scoreOrder.getBuyAvgPrice(),
                                        futureOrder.getDirect(), LTConstants.TRADE_OFFSET_CLOSE, LTConstants.TRADE_ORDER_TYPE_MARKET, LTConstants.FUND_TYPE_SCORE,
                                        scoreOrder.getCreateDate().getTime());
                ...
                for (String productName : productNames) {
                    ...
                    for (FutureOrderBean order : orders) {
                        try {
                            ...
                            orderBean.setFundType(LTConstants.FUND_TYPE_SCORE);
                            getScoreOrderSellTradeService().doSell(orderBean);
    ...
    private class LoopPerRiskTask implements Runnable {
        ...
        public void run() {
            ...
            loadAllOrdersFromDB(PlateEnum.getValueByName(plateName));
        }
