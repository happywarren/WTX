...
public class ScoreOrderSellTradeServiceImpl extends AbstractScoreOrderTradeSerivce implements IOrderSellTradeService {
	 ...
	 * @author XieZhibing
	 * @date 2016年12月12日 下午8:44:03
	 * @see IOrderSellTradeService#doSell(OrderVo)
	 * @param orderVo
	 * @throws LTException
	 */
	@Override
	public void doSell(OrderVo orderVo) throws LTException {
		...
		if(TradeUtils.isSuccess(redisTemplate,"", orderId)){
			...
			removeRiskControlOrder(scoreOrder);
		...
		//获取用户信息
		UserBaseInfo info = userApiService.findUserByUserId(scoreOrder.getUserId());
	...
	public void doSell(String userId, String productCode) throws LTException {
		...
		for(OrderScoreInfo orderScoreInfo : orderScoreInfoList){
			...
			if(TradeUtils.isSuccess(redisTemplate,"", orderScoreInfo.getOrderId())){
				OrderScoreInfoCache.remove(orderScoreInfo.getOrderId());
				removeRiskControlOrder(orderScoreInfo);
...
//				throw new LTException(LTResponseCode.TDJ0002);
				//订单正在卖出中
				logger.error("订单正在卖出中  orderScoreInfo:{} ",JSONObject.toJSON(orderScoreInfo));
				continue;
			}
	...
	private OrderScoreInfo queryScoreOrdersInfo(OrderVo orderVo) throws LTException {
		...
		logger.info("=====执行queryScoreOrdersInfo=scoreOrder={}====",JSONObject.toJSONString(scoreOrder));
		...
		}else if(scoreOrder.getPlate() == PlateEnum.CONTRACT_FOR_DIFFERENCE.getValue()){
			quotePrice = tradeServer.getContractTrade().getQuotePrice(scoreOrder.getProductCode()).getLastPrice();
		}
	...
	private void removeRiskControlOrder(OrderScoreInfo scoreOrdersInfo){
		...
		} else if(PlateEnum.CONTRACT_FOR_DIFFERENCE.getValue() == scoreOrdersInfo.getPlate()) {
			//删除监控队列
			getTradeServer().getContractTrade().removeRiskControlOrder(riskBean);
			logger.info("[差价合约]将订单移除监控队列, displayId:{}", scoreOrdersInfo.getOrderId());
		}
