...
public abstract class AbstractScoreOrderTradeSerivce implements Serializable {
	...
	public void sendBuyEntrustMessage(OrderScoreInfo scoreOrder, OrderScoreEntrustInfo entrustRecord) {
		...
		try {
			//内盘开仓
			if(scoreOrder.getPlate().intValue() == PlateEnum.INNER_PLATE.getValue()){
				//普通用户报单
			...
			//外盘开仓
			else if(scoreOrder.getPlate().intValue() == PlateEnum.OUTER_PLATE.getValue()){
				//普通用户报单
			...
			//差价合约开仓
			else if(scoreOrder.getPlate().intValue() == PlateEnum.CONTRACT_FOR_DIFFERENCE.getValue()){
				//普通用户报单
			...
			//加入订单与委托号关联关系队列
			TradeTimeOut.entrustMapPut(scoreOrder.getOrderId(), entrustRecord.getEntrustId(),scoreOrder.getPlate().intValue());
		} catch (Exception e) {
			//退款并持久化失败订单
	...
	public void sendSellEntrustMessage(OrderScoreInfo scoreOrder, OrderScoreEntrustInfo entrustRecord) {
		...
		try {
			...
			//差价合约平仓
				...
				logger.info("差价合约-积分平仓委托请求已发出, entrustId:{}, productCode:{}, exchangeCode:{}",
						entrustRecord.getEntrustId(), entrustRecord.getProductCode(), entrustRecord.getExchangeCode());
			}
	...
	public void dealMoveStopLoss(OrderScoreInfo scoreOrdersInfo,OrderVo orderVo){
		...
		if(sentinelPrice == null){
			if(PlateEnum.INNER_PLATE.getValue() == scoreOrdersInfo.getPlate().intValue()){
			    mbean = tradeService.getInnerFutureTrade().getMutableStopLossOrder(scoreOrdersInfo.getProductCode(), scoreOrdersInfo.getOrderId());
				
			} else if(PlateEnum.OUTER_PLATE.getValue() ==  scoreOrdersInfo.getPlate().intValue()){
				mbean = tradeService.getOuterFutureTrade().getMutableStopLossOrder(scoreOrdersInfo.getProductCode(), scoreOrdersInfo.getOrderId());
			}else if(PlateEnum.CONTRACT_FOR_DIFFERENCE.getValue() ==  scoreOrdersInfo.getPlate().intValue()){
				mbean = tradeService.getContractTrade().getMutableStopLossOrder(scoreOrdersInfo.getProductCode(), scoreOrdersInfo.getOrderId());
			}
