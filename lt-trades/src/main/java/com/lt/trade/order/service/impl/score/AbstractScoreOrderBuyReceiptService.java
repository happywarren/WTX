...
public abstract class AbstractScoreOrderBuyReceiptService extends
	...
	public void doPersist2012(OrderScoreEntrustInfo entrustRecord) {
		...
		//委托实体入库
		this.addEntrustInfo(entrustRecord);
		
	}


	public void addEntrustInfo(OrderScoreEntrustInfo entrustRecord){
	...
	public void calculateStopLossProfitPrice(OrderScoreInfo scoreOrdersInfo, Integer scale) {		
		...
		// 重新计算止损价
		double stopLossPrice = TradeUtils.stopLossPrice(scoreOrdersInfo.getTradeDirection(), scoreOrdersInfo.getBuyAvgPrice(), scoreOrdersInfo.getPerStopLoss(), jumpPrice, jumpValue);
		//小数位处理
		stopLossPrice = DoubleTools.scaleFormat(stopLossPrice, scale);
		scoreOrdersInfo.setStopLossPrice(stopLossPrice);
		
		//重新计算止盈价
		double stopProfitPrice = TradeUtils.stopProfitPrice(scoreOrdersInfo.getTradeDirection(), scoreOrdersInfo.getBuyAvgPrice(), scoreOrdersInfo.getPerStopProfit(), jumpPrice, jumpValue);
		//小数位处理
