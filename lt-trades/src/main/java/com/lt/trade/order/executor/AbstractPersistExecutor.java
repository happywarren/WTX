...
public abstract class AbstractPersistExecutor implements IPersistExecutor {
	 ...
	 * @see IPersistExecutor#put(FutureMatchWrapper)
		...
		try {
			...
			case 2012:
				...
				if(set.contains(errorBean.getPlatformId())){
					OrderScoreEntrustInfo scoreEntrustRecord = OrderScoreEntrustInfoCache.get(String.valueOf(errorBean.getPlatformId()));
					if(StringTools.isNotEmpty(scoreEntrustRecord)){
				...
				executorService.execute(new Runnable() {
					...
					public void run() {
						try {
							getScoreOrderPersistService().doPersist2012(bean_2012);
			...
			case 2002:
				...
				executorService.execute(new Runnable() {
					...
					public void run() {
						try {
							getScoreOrderPersistService().doPersist2002(bean_2002);
			...
			case 2003:
				...
				if(set.contains(matchBean.getPlatformId())){
					OrderScoreEntrustInfo scoreEntrustRecord = OrderScoreEntrustInfoCache.get(String.valueOf(matchBean.getPlatformId()));
					if(StringTools.isNotEmpty(scoreEntrustRecord)){
						getQueue().put(matchWrapper);
					}
				...
				executorService.execute(new Runnable() {
					...
					public void run() {
						try {
							getScoreOrderPersistService().doPersist2003(bean_2003);
			...
			case 2004:
				...
				executorService.execute(new Runnable() {
					...
					public void run() {
						try {
							getScoreOrderPersistService().doPersist2004(bean_2004);
	 ...
	 * @return
	 */
	public abstract IScoreOrderPersistService getScoreOrderPersistService() ;
}
