...
public class NextPeriodRateCaluate {
     ...
     * 积分订单信息
     */
    private List<NextPeroidOrderInfo> scoreOrderList;
    /**
     * 类型的节假日
     */
    ...
    private ICoinPositionSumService scoreCoinPositionSumService;

    private Map<String, CoinPositionCountBO> scorePositionMap = new HashMap<>();
    ...
    public void run() {
        try {
            ...
            init();
            caluateScoreOrder(scoreOrderList);
            sendMessageToUser();
...
//		
        ...
        scoreOrderList = deferServiceImpl.findAllScoreOrderByCode(futures);
        logger.info("查询所有的订单数据为:{}", JSONObject.toJSONString(scoreOrderList));

        //差价合约持仓
        try {
            List<DigitalCoinPosition> scoreDataList = scoreCoinPositionSumService.getAllBuySellPosition();
            for (DigitalCoinPosition digitalCoinPosition : scoreDataList) {
                String proCode = digitalCoinPosition.getProductCode();
                ...
                scorePositionMap.put(investorId + proCode, coinPositionCountBO);
            }
    ...
    public void initParam() {
        ...
        scoreOrderList = new ArrayList<NextPeroidOrderInfo>();
    ...
    public void caluateScoreOrder(List<NextPeroidOrderInfo> orderList) {
        for (NextPeroidOrderInfo nextPeroidOrderInfo : orderList) {
            ...
            fee = DoubleTools.mul(fee, getScoreMultiple(plate, nextPeroidOrderInfo.getTradeDirection(), nextPeroidOrderInfo.getInvestorId(), nextPeroidOrderInfo.getProductCode()));
            ...
            if (userScore.containsKey(userId)) {
                balance = userScore.get(userId);
            } else {
                FundMainScore mainScore = fundAccountService.queryFundMainScore(userId);
                userScore.put(userId, mainScore.getBalance());
                balance = mainScore.getBalance();
            }

            try {
                fundAccountService.updateScoreByPeriodFee(userId, nextPeroidOrderInfo.getProductName(), nextPeroidOrderInfo.getDisplayId(), fee);
                operateLog.append(UserContant.ORDER_DEFER_FEE_LOG.replace("value", String.valueOf(fee)));
            } catch (Exception e) {
                e.printStackTrace();
                logger.info("订单执行积分递延扣款失败orderid：{}", nextPeroidOrderInfo.getOrderId());
                continue;
            }

            Date sysSetSaleDate = toNextPeroidMethod(nextPeroidOrderInfo, FundTypeEnum.SCORE.getValue());
            ...
            userScore.put(userId, DoubleTools.sub(balance, fee));
            try {
                if (0 > balance) {
                    orderInfo.setDeferStatus(0);
                }
                orderService.updateScoreOrderInfo(orderInfo);
    ...
    private double getScoreMultiple(Integer plate, Integer tradeDirection, String investorId, String proCode) {
        double multiple = 1D;
        try {
            CoinPositionCountBO coinPositionCountBO = scorePositionMap.get(investorId + proCode);
            logger.info("积分 券商: {} 多单: {} 空单: {} ", investorId, coinPositionCountBO.getBuyCount(), coinPositionCountBO.getSellCount());
            //差价合约
        ...
        logger.info("积分 券商: {} plate: {} tradeDirection: {} multiple: {} ", investorId, plate, tradeDirection, multiple);
        return multiple;
    }
...
//						 flag = chkIfTradeDate(DateTools.addDay(date, 1),null,futureType,i);
     ...
     *
     * @param shortCode      产品编码， 短 CL
     * @param date           系统未顺延时间
     * @param settleMentDate 清仓时间
     * @return
     * @throws
     * @return: Date
     * @author yuanxin
    ...
    public ICoinPositionSumService getScoreCoinPositionSumService() {
        return scoreCoinPositionSumService;
    }

    public void setScoreCoinPositionSumService(ICoinPositionSumService scoreCoinPositionSumService) {
        this.scoreCoinPositionSumService = scoreCoinPositionSumService;
    }
}
