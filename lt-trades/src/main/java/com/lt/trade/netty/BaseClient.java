...
public class BaseClient implements Runnable {
    ...
    private OnClientStartupListener startupListener_;
    private volatile boolean isChannelPrepared_;

    private final static int MAX_MESSAGE_LENGTH = 8192;

    public BaseClient(String host, int port, OnMessageListener messageListener, OnClientStartupListener startupListener) {
        host_ = host;
        port_ = port;
            ...
            f.addListener(new GenericFutureListener<Future<? super Void>>() {
                ...
                public void operationComplete(Future<? super Void> future) throws Exception {
                    if (future.isSuccess()) {
                        ...
                        startupListener_.onCompletion(true, BaseClient.this);
                        LOGGER.info("与服务器{}:{}连接建立成功...", host_, port_);
                    } else {
                        isChannelPrepared_ = false;
                        startupListener_.onCompletion(false, BaseClient.this);
