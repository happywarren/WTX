...
public abstract class EClient {
    ...
    // 	7 = The new execDetails event returned for an trade filled status and reqExecDetails
    ...
    // 10 = Added 'serverId' to the 'open trade' & 'trade status' events.
    //      We send back all the API open orders upon connection.
    //      Added new methods reqAllOpenOrders, reqAutoOpenOrders()
    //      Added FA support - reqExecution has filter.
    //                       - reqAccountUpdates takes acct code.
    // 11 = Added permId to openOrder event.
    // 12 = requsting open trade attributes ignoreRth, hidden, and discretionary
    // 13 = added goodAfterTime
    // 14 = always send size on bid/ask/last tick
    // 15 = send allocation description string on openOrder
    // 16 = can receive account name in account and portfolio updates, and fa params in openOrder
    // 17 = can receive liquidation field in exec reports, and notAutoAvailable field in mkt data
    // 18 = can receive good till date field in open trade messages, and request intraday backfill
    ...
    // 24 = can receive volatility trade parameters in open trade messages
	// 25 = can receive HMDS query start and end times
	// 26 = can receive option vols in option market data messages
	// 27 = can receive delta neutral trade type and delta neutral aux price in place trade version 20: API 8.85
	// 28 = can receive option model computation ticks: API 8.9
	// 29 = can receive trail stop limit price in open trade and can place them: API 8.91
	// 30 = can receive extended bond contract def, new ticks, and trade count in bars
	// 31 = can receive EFP extensions to scanner and market data, and combo legs on open orders
	//    ; can receive RT bars
	// 32 = can receive TickType.LAST_TIMESTAMP
	//    ; can receive "whyHeld" in trade status messages
	// 33 = can receive ScaleNumComponents and ScaleComponentSize is open trade messages
	// 34 = can receive whatIf orders / trade state
	...
	// 54 = can receive scale trade fields (PriceAdjustValue, PriceAdjustInterval, ProfitOffset, AutoReset,
	...
	// 66 = can receive randomize size and randomize price trade fields
...
//    public EReader reader()             { return m_reader; }
    ...
    public synchronized void placeOrder( int id, Contract contract, Order order) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
        	if (order.scalePriceIncrement() > 0 && order.scalePriceIncrement() != Double.MAX_VALUE) {
        		if (order.scalePriceAdjustValue() != Double.MAX_VALUE ||
        			...
        			error(id, EClientErrors.UPDATE_TWS,
        				"  It does not support Scale trade parameters: PriceAdjustValue, PriceAdjustInterval, " +
        ...
        if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
        	if (!order.orderComboLegs().isEmpty()) {
                for( OrderComboLeg orderComboLeg : order.orderComboLegs() ) {
        			...
        			error(id, EClientErrors.UPDATE_TWS,
        				"  It does not support per-leg prices for trade combo legs.");
        			return;
        			}
        		}
        	}
        ...
        if (m_serverVersion < MIN_SERVER_VER_ORDER_SOLICITED) {
        	if (order.solicited()) {
        		error(id, EClientErrors.UPDATE_TWS,
                        "  It does not support trade solicited parameter.");
                return;
        	}
        }
        ...
        try {
            ...
            if( m_serverVersion >= 5 ) {
                ...
                if (m_serverVersion < 38) {
                	// will never happen
                	b.send(/* trade.m_ignoreRth */ false);
                }
                else {
            ...
            // Send trade combo legs for BAG requests
            if(m_serverVersion >= MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
           ...
           if (m_serverVersion >= 18) { // institutional short sale slot fields.
               ...
               b.send( order.designatedLocation()); // only populate when trade.m_shortSaleSlot = 2.
           }
           if (m_serverVersion >= MIN_SERVER_VER_SSHORTX_OLD) {
