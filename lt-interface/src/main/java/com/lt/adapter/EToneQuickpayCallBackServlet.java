...
public class EToneQuickpayCallBackServlet extends HttpServlet{

    private Logger logger = LoggerFactory.getLogger(Logger.class);

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        logger.info("【易通快捷支付回调】"+req.getRemoteAddr());
        logger.info("=====[易通快捷充值渠道],收到支付结果通知================");
        ...
        String notifyParam =  paramBuilder.substring(1);
        logger.info("易通快捷支付接收到参数:{}",notifyParam);
        OutputStream oStream =  resp.getOutputStream();
        try {
            MainOperator mainOperator = new MainOperator();
          //  mainOperator.etoneQuickOperator(paramsMap);
