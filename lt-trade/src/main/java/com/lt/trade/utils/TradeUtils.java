...
public class TradeUtils implements Serializable {
     ...
     * @return
     * @author XieZhibing
     * @date 2016年12月9日 下午9:20:52
     */
    public synchronized static Integer makeEntrustCodeId() {
        Date date = new Date();
        String key = MMdd_FORMAT.format(date);
        if (key.startsWith("0")) {
            key = key.substring(1);
        }

        if (entrustCodeMap.isEmpty() || !entrustCodeMap.containsKey(key)) {
            Integer value = 0;

            //获取委托单数据接口
            OrderCashEntrustInfoDao orderCashEntrustInfoDao = (OrderCashEntrustInfoDao) SpringUtils.getBean("orderCashEntrustInfoDao");
            if (orderCashEntrustInfoDao == null) {
                throw new LTException("系统异常: 获取不到委托单数据接口");
            }

            //查询最后一个报给交易所的委托单号
            String entrustId = orderCashEntrustInfoDao.queryLastEntrustId();
            if (entrustId != null) {
                String code = String.valueOf(entrustId);
                if (code.length() > 6) {
                    String md = code.substring(0, code.length() - 6);
                    if (!key.equals(md)) {
                        value = Integer.valueOf(key + "000000");
                    } else {
                        value = Integer.valueOf(entrustId) + 1;
                    }
                }
            } else {
                value = Integer.valueOf(key + "000000");
            }
            entrustCodeMap.put(key, value);
        } else {
            Integer value = entrustCodeMap.get(key);
            entrustCodeMap.clear();
            entrustCodeMap.put(key, value + 1);
        }

        return entrustCodeMap.get(key);
    }
