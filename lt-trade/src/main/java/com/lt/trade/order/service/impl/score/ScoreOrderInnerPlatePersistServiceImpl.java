...
public class ScoreOrderInnerPlatePersistServiceImpl implements IScoreOrderPersistService {
	...
	public void doPersist2003(FutureMatchBean matchBean) {
		...
		if(tradeType == TradeTypeEnum.BUY.getValue()) {
			...
			//成交结果数据持久化
			scoreOrderInnerBuyReceiptServiceImpl.doPersist2003(matchBean, ScoreEntrustRecord);
		...
		//卖多、卖空
			...
			//成交结果数据持久化
			scoreOrderInnerSellReceiptServiceImpl.doPersist2003(matchBean, ScoreEntrustRecord);
	...
	public void doPersist2004(FutureMatchBean matchBean) {
		...
		try {
			...
			if(tradeType == TradeTypeEnum.BUY.getValue()) {			
				...
				//成交结果数据持久化
				scoreOrderInnerBuyReceiptServiceImpl.doPersist2004(matchBean, ScoreEntrustRecord);
			...
			//卖
				...
				//成交结果数据持久化
				scoreOrderInnerSellReceiptServiceImpl.doPersist2004(matchBean,ScoreEntrustRecord);
