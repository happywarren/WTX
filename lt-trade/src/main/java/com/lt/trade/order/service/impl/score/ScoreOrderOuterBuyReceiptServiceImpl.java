...
public class ScoreOrderOuterBuyReceiptServiceImpl extends AbstractScoreOrderBuyReceiptService {
	...
	public void doPersist2003(FutureMatchBean matchBean, OrderScoreEntrustInfo entrustRecord) {
		...
		//设置C++报单账户、、若启用这一块后台券商权重配置会无效
		String investorId = LTConstant.DEFAULT_INVESTOR_ID+"";	
		ScoreOrdersInfo.setInvestorId(investorId);
		ScoreOrdersInfo.setSecurityCode(matchBean.getInvestorId());
		Integer accountId = investorFeeCfgApiService.getInvestorAccountId(matchBean.getInvestorId());
		ScoreOrdersInfo.setAccountId(accountId);
